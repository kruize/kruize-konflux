...
#            Build Docker Image
##########################################################
FROM registry.access.redhat.com/ubi10/ubi-minimal:10.1-1769521234 as mvnbuild-jdk25
ARG USER=autotune
ARG AUTOTUNE_HOME=/home/$USER

RUN microdnf update --setopt=install_weak_deps=0 --setopt=tsflags=nodocs -y \
    && microdnf --setopt=install_weak_deps=0 --setopt=tsflags=nodocs install -y java-25-openjdk-devel \
...
FROM registry.access.redhat.com/ubi10/ubi-minimal:10.1-1769521234
...
RUN microdnf update --setopt=install_weak_deps=0 --setopt=tsflags=nodocs -y \
    && microdnf --setopt=install_weak_deps=0 --setopt=tsflags=nodocs install -y tzdata openssl ca-certificates fontconfig \
